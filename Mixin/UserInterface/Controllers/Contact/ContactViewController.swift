    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0 {
            if contacts.isEmpty {
                ...
                view.label.text = R.string.localizable.contact_title().uppercased()
                return view
            }
        } else if isPhoneContactAuthorized {
            let view = tableView.dequeueReusableHeaderFooterView(withIdentifier: ReuseId.header) as! ContactHeaderView
            if section == 1 {
                view.label.text = R.string.localizable.contact_phone_contacts()
            } else {
    ...
    override func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        if section == 0 {
            ...
            if isPhoneContactAuthorized {
                ...
                view.text = R.string.localizable.contact_phone_contact_summary()
                return view
            }
        }
    }
...
extension ContactViewController: PhoneContactCellDelegate {
    ...
    func phoneContactCellDidSelectInvite(_ cell: PhoneContactCell) {
        ...
        if MFMessageComposeViewController.canSendText() {
            ...
            vc.body = R.string.localizable.contact_invite()
            vc.recipients = [phoneContact.phoneNumber]
            vc.messageComposeDelegate = self
            present(vc, animated: true, completion: nil)
        } else {
            let inviteController = UIActivityViewController(activityItems: [R.string.localizable.contact_invite()],
