...
class ImagePickerController: NSObject {
    ...
    private lazy var selectSourceController: UIAlertController = {
        ...
        if UIImagePickerController.isSourceTypeAvailable(.camera) {
            controller.addAction(UIAlertAction(title: R.string.localizable.action_camera(), style: .default, handler: { [weak self] (_) in
                self?.presentCamera()
            }))
        }
        controller.addAction(UIAlertAction(title: R.string.localizable.action_choose_photo(), style: .default, handler: { [weak self] (_) in
            self?.presentPhoto()
        }))
    ...
    func presentCamera() {
        ...
        if authorization == .authorized || authorization == .notDetermined {
            ...
            viewController.alert(R.string.localizable.permission_denied_camera())
        }
    }
